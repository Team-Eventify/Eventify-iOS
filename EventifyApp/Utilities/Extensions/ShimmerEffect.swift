...
extension View {
    @ViewBuilder func shimmer(isActive: Bool, configuration: ShimmerEffect.Configuration = .default) -> some View {
        if isActive {
 ...
 @ViewBuilder func shimmer(ifNil optional: Optional<Any>, configuration: ShimmerEffect.Configuration = .default) -> some View {
  if optional == nil {
   modifier(ShimmerEffect(configuration: configuration))
  } else {
   self
        ...
        content
            ...
            .overlay {
                Rectangle()
                    ...
                    .animation(.linear(duration: configuration.speed).repeatForever(autoreverses: false), value: moveTo)
            }
    }
}
